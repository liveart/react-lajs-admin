...
export function* createColor(action) {
  try {
    ...
    yield dispatch({type: actionTypes.COLOR_OPERATION_SUCCESS, message: entityName + ' ' + MESSAGE_ENTITY_CREATED});
...
export function* editColor(action) {
  try {
    ...
    yield dispatch({type: actionTypes.COLOR_OPERATION_SUCCESS, message: entityName + ' ' + MESSAGE_ENTITY_UPDATED});
    yield dispatch({type: actionTypes.FETCH_COLORS});
  } catch (e) {
    yield dispatch({type: actionTypes.COLOR_OPERATION_FAILURE, message: e.message});
  }
...
export function* deleteColor(action) {
  try {
    ...
    yield dispatch({type: actionTypes.COLOR_OPERATION_SUCCESS, message: entityName + ' ' + MESSAGE_ENTITY_DELETED});
    yield dispatch({type: actionTypes.FETCH_COLORS});
  } catch (e) {
    yield dispatch({type: actionTypes.COLOR_OPERATION_FAILURE, message: e.message});
  }
